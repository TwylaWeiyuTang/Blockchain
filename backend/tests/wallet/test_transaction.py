...
def test_transaction():
    ...
    # to check if the balance is updated after sending the money
...
def test_transaction_exceeds_balance():
    with pytest.raises(Exception, match='Amount exceeds balance'):
        Transaction(Wallet(), 'recipient', 9001) # an instance
...
def test_transaction_update():
    ...
    assert Wallet.verify(
        ...
        transaction.output,
        transaction.input['signature']
    ) # to check if transaction updates signature
...
def test_valid_transaction_with_invalid_outputs():
    sender_wallet = Wallet()
    transaction = Transaction(sender_wallet, 'recipient', 50)
    transaction.output[sender_wallet.address] = 9001 # give more currency than they really have
...
def test_valid_transaction_with_invalid_signature():
    ...
    # create a new invalid signature here by using the same data 'transaction.output'
